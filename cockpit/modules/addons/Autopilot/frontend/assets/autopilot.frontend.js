(function(APP, d, dom){
    ...
    APP.init = function() {
        ...
        d.body.insertAdjacentHTML('afterend', [
            '<div id="autopilot-frontend-wrapper" class="autopilot-frontend-wrapper">',
                '<div class="autopilot-frontend-container">',
                    '<a class="autopilot-close">&times;</a>',
                    '<iframe id="autopilot-iframe" frameborder="0"></iframe>',
                '</div>',
            '</div>',
            '<a id="autopilot-toggler" class="autopilot-toggler">',
                '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" x="0px" y="0px" viewBox="0 0 40 40" enable-background="new 0 0 40 40" xml:space="preserve" height="30px" width="30px"><path  fill="#fff" clip-rule="evenodd" d="M8.5,12.5v3h23v-3H8.5z M8.5,21.5h23v-3h-23V21.5z M8.5,27.5h23  v-3h-23V27.5z"/></svg>',
            '</a>']
        .join(''));

        APP.toogler   = d.getElementById('autopilot-toggler');
        APP.wrapper   = d.getElementById('autopilot-frontend-wrapper');
        APP.iframe    = d.getElementById('autopilot-iframe');
        APP.optsview  = d.getElementById('autopilot-options');

        APP.toogler.addEventListener("click", function() {
            APP.open();
        });

        APP.wrapper.addEventListener("click", function() {
            location.reload();
            APP.close();
        });
    };
    ...
    dom = cash = $ = function (selector, context) {
