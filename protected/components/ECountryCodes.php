...
class ECountryCodes extends CComponent
{
	 ...
	 */
	private $_geoUsername = '__construct';
	...
	protected static $_eCountryCodes;
	...
	public function __construct($config = array())
	{
	   ...
	   self::$_eCountryCodes = $this;
	}

	/**
	 * Static function to create a instance of ECountrieCodes
	 *
	 * @param array $config
	 * @return ECountryCodes
	 * @since 2.1
	 */
	public static function instance($config = array())
	{
	  	if (self::$_eCountryCodes === null)
		    self::$_eCountryCodes = new ECountryCodes($config);

		return self::$_eCountryCodes;
	}


	/**
	 * Static function to get a specific country info
	 * Usage:
	 * The code
	 * $countryName = ECountryCodes::getCountryValue('countryCode','AT','countryName','de');
	 * will return '�sterreich', the countryname from countrycode 'AT' in german
	 *
	 ...
	 *
	 *
	 * @param string $keyField the key property
	 * @param string $keyValue the key value for the result
	 * @param string $valueField the value property
	 * @return string
	 * @since 2.1
	 */
	public static function getCountryValue($keyField,$keyValue,$valueField,$language=null)
	{
		$countryCodes = self::instance();
		$countryCodes->key = $keyField;
		$countryCodes->value = $valueField;

		if (isset($language))
			$countryCodes->language = $language;

		$data = $countryCodes->getCodesArray();
		return array_key_exists($keyValue,$data) ? $data[$keyValue] : null;
	}
	...
	public function getGeo()
	{
	  if (!isset($this->_geo))
      {
		Yii::import('ext.EGeoNameService.*');
	...
	/** 
	 * Special method for defining countrucodes files path. Works with "Yii:app()" method (if nesesary)
	 * 
	 * @return string
	 */
	protected function getFilePath()
	{
	    return YII::app()->getBasePath().DIRECTORY_SEPARATOR.'data'.DIRECTORY_SEPARATOR.'components'.DIRECTORY_SEPARATOR.get_class($this);
	}
